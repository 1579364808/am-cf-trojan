...
'www.gco.gov.qa#QA',
'www.4chan.org#US',
'www.baipiao.eu.org#EU',
'time.is#IS',
'icook.hk#HK',
'www.zsu.gov.ua#UA',
'www.shopify.com#CA',
'time.cloudflare.com#US',
'www.boba88slot.com#US',
'www.whatismyip.com#US',
'www.okcupid.com#US',
'www.glassdoor.com#US',
'www.visa.co.jp#JP',
'www.iakeys.com#US',
'log.bpminecraft.com#US',
'www.csgo.com#US',
'www.digitalocean.com#US',
'www.visa.com#US',
'www.visa.com.tw#TW',
'xn--b6gac.eu.org#EU',
'icook.tw#TW',
...
'www.visa.com.sg#SG',
'gur.gov.ua#UA',
'www.visakorea.com#KR',
'edtunnel-dgp.pages.dev#US',
'shopify.com#CA',
'www.gov.se#SE',
'www.udacity.com#US',
'www.hugedomains.com#US',
'cdn-b100.xn--b6gac.eu.org#EU'
];
let noTLS = 'false';
let sl = 5;
		...
		try {
			...
			switch (url.pathname.toLowerCase()) {
				...
				case `/${userID}`: {
					...
					const trojanConfig = await getTROJANConfig(userID, host, userAgent, url);
...
/** -------------------processing logic-------------------------------- */
/**
 * Handles TROJAN over WebSocket requests by creating a WebSocket pair, accepting the WebSocket connection, and processing the TROJAN header.
...
async function trojanOverWSHandler(request) {
	...
	const handleStreamData = async (chunk) => {
		...
		const { hasError, message, portRemote = 443, addressRemote = "", rawClientData, addressType } = await parseTrojanHeader(chunk, userID);
 ...
 * @param {Uint8Array} trojanResponseHeader The TROJAN response header.
 * @param {function} log The logging function.
 * @returns {Promise<void>} The remote socket.
 */
async function handleTCPOutBound(remoteSocket, addressRemote, portRemote, rawClientData, webSocket, trojanResponseHeader, log, addressType,) {
	...
	async function retry() {
		...
		remoteSocketToWS(tcpSocket, webSocket, trojanResponseHeader, null, log);
	}
	...
	// remote--> ws
	remoteSocketToWS(tcpSocket, webSocket, trojanResponseHeader, retry, log);
}
...
// https://xtls.github.io/development/protocols/trojan.html

/**
 * Processes the TROJAN header buffer and returns an object with the relevant information.
 * @param {ArrayBuffer} trojanBuffer The TROJAN header buffer to process.
 * @param {string} userID The user ID to validate against the UUID in the TROJAN header.
 * @returns {{
 *  hasError: boolean,
 *  message?: string,
 ...
 *  portRemote?: number,
 *  rawDataIndex?: number,
 *  trojanVersion?: Uint8Array,
 *  isUDP?: boolean
 * }} An object with the relevant information extracted from the TROJAN header buffer.
 */
async function parseTrojanHeader(buffer, userID) {
 ...
 * @param {ArrayBuffer | null} trojanResponseHeader The TROJAN response header.
 * @param {(() => Promise<void>) | null} retry The function to retry the connection if it fails.
 * @param {(info: string) => void} log The logging function.
 * @returns {Promise<void>} A Promise that resolves when the conversion is complete.
 */
async function remoteSocketToWS(remoteSocket, webSocket, trojanResponseHeader, retry, log) {
	// remote--> ws
	let remoteChunkCount = 0;
	let chunks = [];
	/** @type {ArrayBuffer | null} */
	let trojanHeader = trojanResponseHeader;
		...
		.pipeTo(
			new WritableStream({
				...
				async write(chunk, controller) {
					...
					if (trojanHeader) {
						webSocket.send(await new Blob([trojanHeader, chunk]).arrayBuffer());
						trojanHeader = null;
					} else {
